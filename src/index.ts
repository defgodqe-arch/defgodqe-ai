...
function json(data: unknown, status = 200, extra: HeadersInit = {}) {
  return new Response(JSON.stringify(data), { status, headers: { "content-type": "application/json; charset=utf-8", ...corsHeaders(), ...extra } });
}
function getClientKey(request: Request) { return request.headers.get("X-Defgodqe-Client") || "anonymous"; }
function checkRateLimit(request: Request) {
  const key = getClientKey(request), now = Date.now(), existing = rateBuckets.get(key);
  if (!existing || existing.reset <= now) { rateBuckets.set(key, { count: 1, reset: now + RATE_WINDOW_MS }); return { ok: true, retryAfter: 0 }; }
  existing.count += 1;
  if (existing.count > RATE_LIMIT) return { ok: false, retryAfter: Math.ceil((existing.reset - now) / 1000) };
...
function normalizeMessages(value: unknown): ChatMessage[] {
  ...
  return value.slice(-MAX_MESSAGES).map((message: any) => ({ role: message?.role, content: String(message?.content || "").slice(0, MAX_MESSAGE_CHARS) })).filter((message): message is ChatMessage => (message.role === "system" || message.role === "user" || message.role === "assistant") && Boolean(message.content));
}
...
function extractText(value: any): string {
  ...
  if (Array.isArray(value.output)) for (const item of value.output) { const found = extractText(item); if (found) return found; }
  if (Array.isArray(value.content)) for (const item of value.content) { const found = extractText(item); if (found) return found; }
  return "";
}
function extractSources(value: any) {
  const results: Array<{ title: string; url: string; domain: string }> = [], seen = new Set<string>();
  function visit(node: any) {
    ...
    for (const annotation of Array.isArray(node.annotations) ? node.annotations : []) {
      ...
      seen.add(url); let domain = ""; try { domain = new URL(url).hostname.replace(/^www\./, ""); } catch {}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/") return json({ success: true, service: "defgodqe-ai", routes: ["/chat", "/web-search", "/generate-image", "/vision", "/tts", "/usage"] });
    ...
    const globalLimit = await env.RATE_LIMITER.limit({ key: `${getClientKey(request)}:${url.pathname}` });
    if (!globalLimit.success) return json({ success: false, error: "Too many requests. Please slow down." }, 429, { "retry-after": "60" });
...
async function handleChat(request: Request, env: Env, stream: boolean) {
  const body = (await request.json()) as { messages?: unknown }, messages = normalizeMessages(body.messages);
...
async function handleWebSearch(request: Request, env: Env) {
  const body = (await request.json()) as { query?: unknown }, query = String(body.query || "").trim().slice(0, 4000);
  ...
  const response = extractText(result); return json({ success: Boolean(response), response, sources: extractSources(result) });
}
async function handleVision(request: Request, env: Env) {
  const body = (await request.json()) as { prompt?: unknown; image?: unknown }, prompt = String(body.prompt || "Describe and analyze this image accurately.").trim().slice(0, 4000), image = String(body.image || "").trim();
  ...
  const rawImage = image.startsWith("data:") && image.includes(",") ? image.split(",", 2)[1] : image;
...
async function handleTts(request: Request, env: Env) {
  const body = (await request.json()) as { text?: unknown; speaker?: unknown }, text = String(body.text || "").trim().slice(0, 5000);
...
async function handleImage(request: Request, env: Env) {
  const body = (await request.json()) as { prompt?: unknown }, prompt = String(body.prompt || "").trim().slice(0, 2048);
